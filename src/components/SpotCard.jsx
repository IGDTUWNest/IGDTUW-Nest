...
export default function SpotCard({ title, description, distance, images, mapsLink, secondaryLinks }) {
  ...
  return (
    <div className="rounded-2xl glass-panel glass-panel-hover flex flex-col overflow-hidden h-full">
      ...
      <div className="relative h-48 sm:h-56 overflow-hidden bg-brand-bg/60 border-b border-white/5">
        ...
        {/* Distance Badge */}
        <div className="absolute top-3 left-3 bg-brand-bg/85 backdrop-blur-sm border border-white/10 px-2.5 py-1 rounded-lg text-xs font-semibold text-brand-pink flex items-center gap-1">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between gap-4">
        ...
        <div className="pt-3 border-t border-white/5 flex flex-wrap items-center gap-x-3 gap-y-2 text-xs">
          {secondaryLinks && secondaryLinks.length > 0 ? (
            <div className="flex flex-wrap items-center gap-2">
              ...
              {secondaryLinks.map((link, idx) => (
                <a
                  ...
                  className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 hover:border-brand-pink hover:text-brand-pink transition duration-200 font-semibold"
                >
                  {link.label}
                </a>
              ))}
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:border-brand-pink hover:bg-brand-pink/10 hover:text-brand-pink transition duration-300 font-semibold w-full justify-center"
            >
